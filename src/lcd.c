...
static void CmdWrite(UInt8 Val);
static void DataWrite(UInt8 Val);
...
void LcdInit(void)
{
	...
	// Initialize the LCD
	CmdWrite(CMD_SEG_NORMAL);		// RAM address SEG output: normal
	CmdWrite(CMD_DISP_OFF);			// display: off
	CmdWrite(CMD_COM_REVERSE);		// COM output scan direction: reverse
	CmdWrite(CMD_BIAS_1_9);			// drive voltage bias ratio: 1/9
	CmdWrite(CMD_ALL_PTS_OFF);		// display all points: off
	CmdWrite(CMD_PWR_CTRL + 7);		// power operating mode: 7
	CmdWrite(CMD_RES_RATIO + 1);	// internal resistor ratio mode: 1:1
	CmdWrite(CMD_ELEC_VOL_REG);		// set electronic volume register
	CmdWrite(0x1F);					// electronic volume register value
	CmdWrite(CMD_DISP_NORMAL);		// display: normal
	CmdWrite(CMD_DISP_ON);			// display: on
...
void LcdClear(void)
{
	...
	for (DispPage = 0; DispPage < LCD_PAGE_HEIGHT; DispPage++)
	{
		...
		CmdWrite(CMD_COL_UPPER + 0);	// RAM column address upper 4 bits: 0
		CmdWrite(CMD_COL_LOWER + 0);	// RAM column address lower 4 bits: 0
		CmdWrite(CMD_PAGE + DispPage);	// RAM page address: DispPage
...
void LcdReverse(Bool ReverseDisplay)
{
	if (ReverseDisplay)
	{
		CmdWrite(CMD_DISP_REVERSE);
	}
	else
	{
		CmdWrite(CMD_DISP_NORMAL);
	}
}
...
{
	static const ObjectType ObjectArray[] =
	{
		{ {0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},		// 1x1 square point
		  1, 1													// width, height
		},
		{ {0x03, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},		// 2x2 square point
		  2, 2													// width, height
		}
	};
	static const UInt8	TopPageMaskArray[]    = {0x00, 0x01, 0x03, 0x07, 0x0f, 0x1f, 0x3f, 0x7f};
	static const UInt8	BottomPageMaskArray[] = {0xff, 0xfe, 0xfc, 0xf8, 0xf0, 0xe0, 0xc0, 0x80};
	static const UInt8 * DispMaskArray = BottomPageMaskArray;
	...
	UInt8 DispMask;
	int ImageCol;
	UInt8 const * ImageBufPtr = ObjectArray[Object].ByteArray;
	int RenderCol;
	UInt8 RenderVal;
	int VertOffset;

	// Limit the object to be within the display
	PosX = PosX < 0 ? 0 : PosX;
	PosX = PosX > LCD_PIXEL_WIDTH - ObjectArray[Object].Width ? LCD_PIXEL_WIDTH - ObjectArray[Object].Width : PosX;
	PosY = PosY < 0 ? 0 : PosY;
	PosY = PosY > LCD_PIXEL_HEIGHT - ObjectArray[Object].Height ? LCD_PIXEL_HEIGHT - ObjectArray[Object].Height : PosY;

	// Determine the starting column, top page, and offset within the page
	DispCol = PosX;
	DispPage = PosY / PAGE_PIXEL_HEIGHT;
	VertOffset = PosY - (DispPage *  PAGE_PIXEL_HEIGHT);

	// Set up the top page and column addresses for the display
	CmdWrite(CMD_COL_UPPER + (DispCol >> 4));
	CmdWrite(CMD_COL_LOWER + (DispCol & 0x0f));
	CmdWrite(CMD_PAGE + DispPage);

	// Render the object image into the top page, retaining other objects within the page
	for (RenderCol = DispCol, ImageCol = 0; ImageCol < ObjectArray[Object].Width; RenderCol++, ImageCol++)
	{
		DispVal = ShadowDisp[DispPage][RenderCol];
		if (Mode == LCD_MODE_CURSOR)
		{
			// XOR the shifted object image with the display contents
			RenderVal = DispVal ^ (ImageBufPtr[ImageCol] << VertOffset);
		}
		else
		{
			// Replace the display contents with the shifted object image
			DispMask = TopPageMaskArray[VertOffset] | (DispMaskArray[ObjectArray[Object].Height] << VertOffset);
			RenderVal = (DispVal & DispMask) | (ImageBufPtr[ImageCol] << VertOffset);
//			RenderVal = (DispVal & (TopPageMask[VertOffset] | (DispMask[ObjectArray[Object].Height] << VertOffset))) | (ImageBufPtr[ImageCol] << VertOffset);
		}
		DataWrite(RenderVal);
		ShadowDisp[DispPage][RenderCol] = RenderVal;
	}

	// Check for the object being completely within the page or on the last page
	if (((VertOffset + ObjectArray[Object].Height) <= PAGE_PIXEL_HEIGHT) || (DispPage == (LCD_PAGE_HEIGHT - 1)))
	{
		return;
	}

	// Set up the bottom page and column addresses for the display
	DispPage++;
	CmdWrite(CMD_COL_UPPER + (DispCol >> 4));
	CmdWrite(CMD_COL_LOWER + (DispCol & 0x0f));
	CmdWrite(CMD_PAGE + DispPage);
	...
	for (RenderCol = DispCol, ImageCol = 0; ImageCol < ObjectArray[Object].Width; RenderCol++, ImageCol++)
	{
		...
		{
			// XOR the shifted object image with the display contents
			RenderVal = DispVal ^ (ImageBufPtr[ImageCol] >> (PAGE_PIXEL_HEIGHT - VertOffset));
		}
		else
		{
			// Replace the display contents with the shifted object image
			DispMask = BottomPageMaskArray[VertOffset] | (DispMaskArray[ObjectArray[Object].Height] >> (PAGE_PIXEL_HEIGHT - VertOffset));
			RenderVal = (DispVal & DispMask) | (ImageBufPtr[ImageCol] >> (PAGE_PIXEL_HEIGHT - VertOffset));
//			RenderVal = (DispVal & (BottomPageMask[VertOffset] | (DispMask[ObjectArray[Object].Height] >> (PAGE_PIXEL_HEIGHT - VertOffset)))) | (ImageBufPtr[ImageCol] >> (PAGE_PIXEL_HEIGHT - VertOffset));
		}
		DataWrite(RenderVal);
		ShadowDisp[DispPage][RenderCol] = RenderVal;
	}
...
int LcdPrintf(int CharRow, int CharCol, char * FmtStrPtr, ...)
{
	...
	CmdWrite(CMD_COL_UPPER + ((CharCol * CHAR_PIXEL_WIDTH) >> 4));		// RAM column address upper 4 bits
	CmdWrite(CMD_COL_LOWER + ((CharCol * CHAR_PIXEL_WIDTH) & 0x0F));	// RAM column address lower 4 bits
	CmdWrite(CMD_PAGE + CharRow);										// RAM page address
